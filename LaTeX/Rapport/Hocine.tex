...
Malheureusement, pour le temps du stage, je n'ai pas obtenu l'accès au serveur, donc je n'ai pas pu utilisé cette partie de code. Je l'ai donc pas utilisé, ni pu développer ma propre version. C'est pourquoi, j'ai décidé de ne pas l'inclure dans mon propre code.\\

% données locales
Donc pour que je puisse travailler, on a mis à ma disposition certains de ces fichiers, ceux des collisions à 250 \GeV. Soient 66 dossiers \Figure{\ref{data:list}} de fichiers \LCIO, où chacun correspond au code du type de processus \Figure{\ref{data:def}}.
